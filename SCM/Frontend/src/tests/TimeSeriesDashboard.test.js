...
test("Input data must be in the correct format.", () => {
    render(<TimeSeriesDashboard data={[{'time':'abcdef', 'data':'123456'}]}/>);
    const errorMessage = screen.queryByText('Incorrect data format. Expected data = [{timestamp: <date_time>, data: <float>}, ...].');
    expect(errorMessage).toBeVisible();
});

// test("Line plot must be displayed.", () => {
//     render(<TimeSeriesDashboard data={[{'time':'abcdef', 'data':'123456'}]}/>);
//     const errorMessage = screen.queryByText('Incorrect data format.');
//     expect(errorMessage).toBeVisible();
// });