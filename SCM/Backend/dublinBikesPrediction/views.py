...
class DublinBikesModel:
    def __new__(cls):
        if not hasattr(cls, '_instance'):
            ...
            cls.model = pickle.load(open("dublinBikesPrediction/assets/RFmodel_lr.pkl", 'rb'))
            cls.df_stations = pd.read_csv('dublinBikesPrediction/assets/STATION ID - BIKE STANDS.csv')
            cls.df_positions = pd.read_csv('dublinBikesPrediction/assets/STATION ID - POSITION.csv')
...
def prepare_query_for_model(weather_forecast=None):
    ...
    df_return = res.copy()
    df_return.drop(['hour', 'minute', 'day', 'dayofweek'], axis=1, inplace=True)
...
def predict(request):
    ...
    return JsonResponse({'prediction': df_return.to_json(), 'positions': positions.to_json()})
