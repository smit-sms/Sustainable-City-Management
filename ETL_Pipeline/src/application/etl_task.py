...
class ETLTask:
    ...
    def __init__(self, 
        name:str, 
        fun_data_load:ETLTaskFunction,
        fun_data_save:ETLTaskFunction,
        ...
        @param repeat_interval: The units of time after which this funciton shall
                                be executed again.
        ...
        @oaram time_run_last_end: Last time this function fininshed running.
    ...
    def __schedule_repeat(self, url:str):
        """ 
        Repeats every x seconds, minutes, hours or days. 
        @param url: URL to where the fastapi is hosted.
        """
        if self.repeat_time_unit == "seconds":
            schedule.every(self.repeat_interval).seconds.do(lambda: self.run(url))
        elif self.repeat_time_unit == "minutes":
            schedule.every(self.repeat_interval).minutes.do(lambda: self.run(url))
        elif self.repeat_time_unit == "hours":
            schedule.every(self.repeat_interval).hours.do(lambda: self.run(url))
        elif self.repeat_time_unit == "days":
            schedule.every(self.repeat_interval).days.do(lambda: self.run(url))
        else:
    ...
    def schedule(self, schedule:schedule, host:str, port:int):
        ...
        if self.repeat_time_unit == "seconds":
            ...
            raise Exception(f'Task "{self.name}: Invalid repeat time unit "{self.repeat_time_unit}".')
        # schedule.every().day.at(time_start_str).do(lambda: self.__schedule_repeat(url))